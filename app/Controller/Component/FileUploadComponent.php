...
{}
  ...
  function _processFile(){
    ...
    if(move_uploaded_file($this->uploadedFile['tmp_name'], $target_path)){
      ...
      $this->_error('FileUpload::processFile() - ' . __("Unable to save temp file to file system.", true));
    ...
    */
    ...
    if($name){
      ...
      if (empty($model) && $this->fileModel) {
        ...
        $this->_error('FileUpload::getModel() - ' . __('Model is not set or could not be found', true));
  ...
  function _checkType(){
    ...
    $this->_error("FileUpload::_checkType() {$this->uploadedFile['type']} " . __(" is not in the allowedTypes array.", true));
  ...
  function _uploadedFileArray(){
    ...
    if($this->uploadDetected && $retval === false){
      ...
      $this->_error("FileUpload: " . __("A file was detected, but was unable to be processed due to a misconfiguration of FileUpload. Current config", true) . " -- fileModel:'{$this->fileModel}' fileVar:'{$this->fileVar}'");
